...
## PDC considers the incoming flow, which is the row-wise sum
def AR_to_H(input_df,freq):
    ...
    for t in range(num_nodes):  # this is the inflow:
        vec_row = H_freq[t, :]
        vec_product = np.inner(np.conjugate(vec_row), vec_row)
        vec_product = np.abs(vec_product)
        for k in range(num_nodes):
            value_tmp = H_freq[t, k]
            value_abs = np.abs(value_tmp) ** 2
            matrix_normalized[k, t] = value_abs / vec_product
...
def PDC_summation():
    ...
    for frame in sz_series:
        pdc_per_frame = Sub_band(input_df=frame,band=[13,30]) # this is whole connected;
        PDC_list.append(pdc_per_frame)
    pdc_sum = sum(PDC_list)
    matrix_total = pdc_sum.div(frame_count)
